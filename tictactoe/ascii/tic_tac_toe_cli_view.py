...
class TicTacToeCLIView:
    ...
    def get_player_input(self, spieler, data):
        ...
        while not valid_reply_received:
            ...
            self.input_text = input(f'      Spieler {spieler}, bitte wähle x→ und y↓ Koordinaten (Werte 0 bis 2): ')
            self.error_text = ""
            if is_not_valid(self.input_text):
                self.error_text = "Ungültige Eingabe " + self.input_text + "! Ein gültiger Eingabewert ist z.B.: 1 1 "
            else:
